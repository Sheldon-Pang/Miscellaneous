...
void* simulate_user_request(void* user_id); /* forward declarations */
balancer* lb; /* variable to store load balancer object */
...
void* simulate_user_request(void* user_id) {
    ...
    *result = -1;
    ...
    int ms = (rand() % 100) * 100000;
    nanosleep((struct timespec[]){{0, ms*100000000}}, NULL);
