...
typedef struct balancer { /* structure for representing the load balancer */
    int number_of_requests; /* track number of jobs received */
    int batch_size;         /* track size of the batch to spawn instance */
    struct job_node* head;  /* track head of a node */
} balancer;

struct job_node {           /* structure to track jobs as they are created, serves as list node */
